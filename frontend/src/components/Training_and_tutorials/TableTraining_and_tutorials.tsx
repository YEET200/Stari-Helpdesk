...
import { loadColumns } from './configureTraining_and_tutorialsCols';
...
const TableSampleTraining_and_tutorials = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: training_and_tutorialsNotify,
    refetch,
  } = useAppSelector((state) => state.training_and_tutorials);
  ...
  useEffect(() => {
    if (training_and_tutorialsNotify.showNotification) {
      notify(
        training_and_tutorialsNotify.typeNotification,
        training_and_tutorialsNotify.textNotification,
      );
    }
  }, [training_and_tutorialsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/training_and_tutorials/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(
      `/training_and_tutorials/training_and_tutorials-view/?id=${id}`,
    );
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/training_and_tutorials/training_and_tutorials-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
  ...
  return (
    <>
      ...
      <BigCalendar
        ...
        entityName={'training_and_tutorials'}
      />
...
export default TableSampleTraining_and_tutorials;
