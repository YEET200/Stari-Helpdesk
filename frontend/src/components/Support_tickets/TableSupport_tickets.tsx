...
import { loadColumns } from './configureSupport_ticketsCols';
...
const TableSampleSupport_tickets = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: support_ticketsNotify,
    refetch,
  } = useAppSelector((state) => state.support_tickets);
  ...
  useEffect(() => {
    if (support_ticketsNotify.showNotification) {
      notify(
        support_ticketsNotify.typeNotification,
        support_ticketsNotify.textNotification,
      );
    }
  }, [support_ticketsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/support_tickets/support_tickets-view/?id=${id}`);
  };
...
export default TableSampleSupport_tickets;
