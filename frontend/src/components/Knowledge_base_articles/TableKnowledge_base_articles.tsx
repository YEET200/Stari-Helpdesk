...
import { loadColumns } from './configureKnowledge_base_articlesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListKnowledge_base_articles from './ListKnowledge_base_articles';

const perPage = 10;

const TableSampleKnowledge_base_articles = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: knowledge_base_articlesNotify,
    refetch,
  } = useAppSelector((state) => state.knowledge_base_articles);
  ...
  useEffect(() => {
    if (knowledge_base_articlesNotify.showNotification) {
      notify(
        knowledge_base_articlesNotify.typeNotification,
        knowledge_base_articlesNotify.textNotification,
      );
    }
  }, [knowledge_base_articlesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/knowledge_base_articles/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(
      `/knowledge_base_articles/knowledge_base_articles-view/?id=${id}`,
    );
  };
  ...
  return (
    <>
      ...
      {knowledge_base_articles && Array.isArray(knowledge_base_articles) && (
        <ListKnowledge_base_articles
...
export default TableSampleKnowledge_base_articles;
