...
module.exports = class UsersDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.community_forum_posts_author =
      await users.getCommunity_forum_posts_author({
        transaction,
      });

    output.community_forum_posts_moderator =
      await users.getCommunity_forum_posts_moderator({
        transaction,
      });

    output.feedback_and_surveys_customer =
      await users.getFeedback_and_surveys_customer({
        transaction,
      });

    output.knowledge_base_articles_author =
      await users.getKnowledge_base_articles_author({
        transaction,
      });

    output.support_tickets_customer = await users.getSupport_tickets_customer({
      transaction,
    });

    output.support_tickets_assigned_agent =
      await users.getSupport_tickets_assigned_agent({
        transaction,
      });
